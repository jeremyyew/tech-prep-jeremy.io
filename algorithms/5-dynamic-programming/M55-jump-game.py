...
1. Iterative, backwards, O(N) space:
    ...
    Let `dp[i]` be a boolean value for whether we can jump from `i` to the last index `N`.
    Let `edges[i]` be the leftmost (smallest) index so far from `i` to `N` such that if we can jump to `edges[i]` (NOT `i`), we can eventually jump to `N`.
